...
export default function Checkout({ cartItem }) {
  ...
  async function orderPlace() {
    ...
    if (!address || !payment) {
      setErrorMessage('Please provide both delivery address and payment method.');
      return;
    }
    const item = { userId, address, payment };
    let result;
    try {
      result = await fetch('http://127.0.0.1:8000/api/orderplace', {
        method: 'POST',
        headers: {
      ...
      if (result) {
        ...
        navigate('/order-success'); // Navigate to OrderSuccessMassage upon successful order placement
        console.log("Navigated to OrderSuccessMassage.");
      }
    } catch (error) {
          ...
          <form>
            ...
            <div className="form-group">
              ...
              <div onChange={onChangeValue}>
                <p><input type="radio" value="online" name="payment" /> Online Payment</p>
                <p><input type="radio" value="cash" name="payment" /> Cash on Delivery</p>
              </div>
            </div>
            {errorMessage && <div className="text-danger mb-2">{errorMessage}</div>}
            <button className="btn btn-info" type="button" onClick={orderPlace}>Confirm Checkout</button>
          </form>
        </div>
      </div>
    </>
